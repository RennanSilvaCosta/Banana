...
public class ControllerReceitaScreen implements Initializable {
    ...
    @FXML
    CurrencyField txtValor = new CurrencyField();

    @FXML
    JFXComboBox<Label> comboBoxCategorias = new JFXComboBox<Label>();

    @FXML
    Label labelCategorias;

    @FXML
    JFXButton btnRepetir, btnAnexar;
    ...
    private void initializeComboboxCategorias() {
        ...
        try {
            for (String labelsItemList : categorias.keySet()) {
                ...
                comboBoxCategorias.getItems().add(labelCategorias);
            }
    ...
    private void initializeButtons() {
        try {
            btnAnexar.setGraphic(new ImageView(new Image(new FileInputStream("C:\\Users\\renna\\IdeaProjects\\banana\\src\\icons\\addreceita\\icon_receita_anexar.png"))));
            btnRepetir.setGraphic(new ImageView(new Image(new FileInputStream("C:\\Users\\renna\\IdeaProjects\\banana\\src\\icons\\addreceita\\icon_receita_repetir.png"))));
        } catch (FileNotFoundException e) {
            e.printStackTrace();
        }
    }
